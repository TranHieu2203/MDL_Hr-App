...
import { ToolbarItem, ToolbarInterface, Terminals } from "src/app/_models/index";
...
export class TerminalsComponent implements OnInit {
  ...
  modelTemp: Terminals = new Terminals();
  editForm!: FormGroup;
  ...
  viewRecord = (event: any) => {
    ...
    this.router.navigate(["/hrms/attendance/list/terminals/", paramAdd]);
  };

  // GetListData
  getListData = (): void => {
    
    this._coreService.execute(this.state, "hr/terminals/GetAll");
  };
  ...
  public dataStateChange(state: DataStateChangeEventArgs): void {
    ...
    let extraParams: any[] = [];
    this._coreService.execute(this.state, "hr/terminals/GetAll", extraParams);
  }
  // Số thứ tự
  ...
  clickToolbar = (itemButton: any): void => {
    ...
    switch (buttonId) {
      case ToolbarItem.ADD:
        this.dialogHeader = "Thêm mới máy chấm công";
      ...
      case ToolbarItem.EXPORT_EXCEL:
        var data = this._coreService.ExecuteToExport('hr/terminals/GetAll','',this.state).subscribe((res: any) => {
          // Lấy ngày hiện tại
          ...
          const exportProperties: ExcelExportProperties = {
            dataSource: res.data,
            columns: [],
            fileName: "thietlapmaychamcong" + "-" + formattedDate + ".xlsx"
          };
      ...
      case ToolbarItem.LOCK:
        if (selectDeletes && selectDeletes.length > 0) {
          ...
          this._coreService
            .Post("hr/terminals/ChangeStatus", ids)
            .subscribe((res: any) => {
  ...
  */
  saveData = () => {
    ...
    if (this.flagState$.value === Consts.add) {
      this._coreService.Post("hr/terminals/add", this.model).subscribe(
        (res: any) => {
      ...
      this._coreService.Post("hr/terminals/Update", this.model).subscribe(
        (res: any) => {
          if (res.statusCode == 400) {
            this.notification.checkErrorMessage(res.message);
          } else {
  ...
  confirmDelete = (status: any): void => {
    if (status === "cancel") {
      ...
      if (lstDeleteIds.length > 0) {
        this._coreService
          .Post("hr/terminals/delete", lstDeleteIds)
