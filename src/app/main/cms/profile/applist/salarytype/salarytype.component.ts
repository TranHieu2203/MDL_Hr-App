...
import { ToolbarItem, ToolbarInterface, SalaryType } from "src/app/_models/index";
...
export class SalaryTypeComponent implements OnInit {
  ...
  modelTemp: SalaryType = new SalaryType();
  editForm!: FormGroup;
  ...
  viewRecord = (event: any) => {
    ...
    this.router.navigate(["/hrms/profile/list/salarytype/", paramAdd]);
  };

  // GetListData
  getListData = (): void => {
    
    this._coreService.execute(this.state, "hr/salarytype/GetAll");
  };
  ...
  public dataStateChange(state: DataStateChangeEventArgs): void {
    ...
    let extraParams: any[] = [];
    this._coreService.execute(this.state, "hr/salarytype/GetAll", extraParams);
  }
  // Số thứ tự
  ...
  clickToolbar = (itemButton: any): void => {
    ...
    switch (buttonId) {
      case ToolbarItem.ADD:
        this.dialogHeader = "Thêm mới bảng lương";
        this.flagState$.next(Consts.add)
        this.model = new SalaryType();
        this.getType();
      ...
      case ToolbarItem.EXPORT_EXCEL:
        var data = this._coreService.ExecuteToExport('hr/salarytype/GetAll','',this.state).subscribe((res: any) => {
          // Lấy ngày hiện tại
          ...
          const exportProperties: ExcelExportProperties = {
            dataSource: res.data,
            columns: [],
            fileName: "bangluong" + "-" + formattedDate + ".xlsx"
          };
  ...
  confirmDelete = (status: any): void => {
    if (status === "cancel") {
      ...
      if (lstDeleteIds.length > 0) {
        this._coreService
          .Post("hr/salarytype/delete", lstDeleteIds)
  ...
  saveData = () => {
    ...
    if (this.flagState$.value === Consts.add) {
      this._coreService.Post("hr/salarytype/add", this.model).subscribe(
        (res: any) => {
      ...
      this._coreService.Post("hr/salarytype/Update", this.model).subscribe(
        (res: any) => {
          if (res.statusCode == 400) {
            this.notification.checkErrorMessage(res.message);
          } else {
