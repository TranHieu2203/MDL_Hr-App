...
export class OtherlistComponent implements OnInit {
  ...
  viewRecord = (event: any) => {
    ...
    this.router.navigate(["/hrms/profile/list/otherlist/", paramAdd]);
  };
  ...
  clickToolbar = (itemButton: any): void => {
    ...
    switch (buttonId) {
      ...
      case ToolbarItem.EXPORT_EXCEL:
        ...
        var data = this._coreService.ExecuteToExport('hr/otherlist/GetAllByType', extraParams, this.state).subscribe((res: any) => {
          // Lấy ngày hiện tại
      ...
      case ToolbarItem.DELETE:
        if (selectDeletes && selectDeletes.length > 0) {
          ...
          for (let i = 0; i < this.modelDelete.length; i++) {
            if (this.modelDelete[i].isActiveText === "Áp dụng") {
              checkHieuLuc = true;
            }
          }
          if (checkHieuLuc) {
  ...
  getTreeView() {
    this._coreService
      .Get("hr/otherlist/GetOtherListTreeView?ModuleId=3")
      .subscribe((res: any) => {
  ...
  getType() {
    ...
    this._coreService
      .Get("hr/otherlist/GetAllType?ModuleId=3")
      .subscribe((res: any) => {
