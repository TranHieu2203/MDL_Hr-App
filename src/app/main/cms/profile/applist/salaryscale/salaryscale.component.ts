...
import { ToolbarItem, ToolbarInterface, SalaryScale } from "src/app/_models/index";
...
export class SalaryScaleComponent implements OnInit {
  ...
  // edit
  model: SalaryScale = new SalaryScale();
  modelTemp: SalaryScale = new SalaryScale();
  editForm!: FormGroup;
  ...
  viewRecord = (event: any) => {
    ...
    this.router.navigate(["/hrms/profile/list/salaryscale/", paramAdd]);
  };

  // Build Toolbar
  buildToolbar = () => {
    const toolbarList = [ToolbarItem.ADD, ToolbarItem.EDIT, ToolbarItem.LOCK];
    this.toolbar = this.globals.buildToolbar("salaryscale", toolbarList);
  };
  // filter checkbox
  ...
  getListData = (): void => {
    ...
    this._coreService.execute(this.state, "hr/salaryscale/GetAll");
  };
  ...
  public dataStateChange(state: DataStateChangeEventArgs): void {
    ...
    let extraParams: any[] = [];
    this._coreService.execute(this.state, "hr/salaryscale/GetAll", extraParams);
  }
  // Số thứ tự
  ...
  clickToolbar = (itemButton: any): void => {
    ...
    switch (buttonId) {
      case ToolbarItem.ADD:
        this.dialogHeader = "Thêm mới thang lương";
        this.model = new SalaryScale();
      ...
      case ToolbarItem.EXPORT_EXCEL:
        var data = this._coreService.ExecuteToExport('hr/salaryscale/GetAll','',this.state).subscribe((res: any) => {
          // Lấy ngày hiện tại
          ...
          const exportProperties: ExcelExportProperties = {
            dataSource: res.data,
            columns: [],
            fileName: "thangluong" + "-" + formattedDate + ".xlsx"
          };
      ...
      case ToolbarItem.LOCK:
        if (selectDeletes && selectDeletes.length > 0) {
          ...
          this._coreService
            .Post("hr/salaryscale/ChangeStatus", ids)
            .subscribe((res: any) => {
  ...
  confirmDelete = (status: any): void => {
    if (status === "cancel") {
      ...
      if (lstDeleteIds.length > 0) {
        this._coreService
          .Post("hr/salaryscale/delete", lstDeleteIds)
  ...
  saveData = () => {
    ...
    if (this.flagState$.value === Consts.add) {
      this._coreService.Post("hr/salaryscale/add", this.model).subscribe(
        (res: any) => {
      ...
      this._coreService.Post("hr/salaryscale/Update", this.model).subscribe(
        (res: any) => {
          if (res.statusCode == 400) {
            this.notification.checkErrorMessage(res.message);
          } else {
