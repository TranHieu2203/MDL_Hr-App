...
export class SalaryRankEditComponent implements OnInit {
    ...
    async.waterfall(
      [
        (cb: any) => {
          if (this.paramId) {
            this._coreService
              .Get("hr/SalaryRank/get?id=" + this.paramId)
              .subscribe((res: any) => {
    ...
    switch (buttonId) {
      case ToolbarItem.BACK:
        ...
        if (this.flagePopup === true) {
          this.router.navigate(["hrms/profile/list/salaryrank"]);
        }
        break;
  ...
  saveData = () => {
    ...
    if (this.flagState$.value === "new") {    
      this._coreService.Post("hr/salaryrank/add", param).subscribe(
        (res: any) => {
          //check error
      ...
      this._coreService.Post("hr/salaryrank/Update", param).subscribe(
        (res: any) => {
          //check error
          if (res.statusCode == 400) {
            this.notification.checkErrorMessage(res.message);
