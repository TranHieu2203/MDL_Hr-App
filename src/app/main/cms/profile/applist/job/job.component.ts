...
export class JobComponent implements OnInit {
  ...
  viewRecord = (event: any) => {
    ...
    this.router.navigate(["/hrms/organize/business/job/", paramAdd]);
  };
  ...
  buildToolbar = () => {
    ...
    this.toolbar = this.globals.buildToolbar("job", toolbarList!);
  };
  // filter checkbox
  ...
  getListData = () => {
    ...
    this._coreService.execute(this.state, "hr/job/jobs");
  };
  ...
  clickToolbar = (itemButton: any): void => {
    ...
    switch (buttonId) {
      case ToolbarItem.ADD:
        this.router.navigate(["/hrms/organize/business/job/new"]);
        break;
      ...
      case ToolbarItem.DELETE:
        if (selectDeletes && selectDeletes.length > 0) {
          ...
          for (let i = 0; i < this.modelDelete.length; i++) {
            if (this.modelDelete[i].status != 0) {
              checkHieuLuc = true;
            }
          }
          if (checkHieuLuc) {
  ...
  confirmDelete = (status: any): void => {
    if (status === "cancel") {
      ...
      if (lstDeleteIds.length > 0) {
        this._coreService
          .Post("hr/job/delete", lstDeleteIds)
