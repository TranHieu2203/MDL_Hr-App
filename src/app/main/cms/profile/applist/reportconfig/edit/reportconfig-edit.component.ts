...
import { ReportConfig } from "src/app/_models/app/cms/index";
...
@Component({
  selector: "app-reportconfig-edit",
  templateUrl: "./reportconfig-edit.component.html",
  styleUrls: ["./reportconfig-edit.component.scss"],
...
export class ReportConfigEditComponent implements OnInit {
  ...
  model: ReportConfig = new ReportConfig();
  modelTemp: ReportConfig = new ReportConfig();
  languages: any;
    ...
    async.waterfall(
      [
        (cb: any) => {
          if (this.paramId) {
            this._coreService
              .Get("package/report/get?id=" + this.paramId)
              .subscribe((res: any) => {
        ...
        (cb: any) => {
          this._coreService.Get("package/module/GetList").subscribe((res: any) => {
            this.lstmoduleId = res.data;
            cb();
          });
        },
    ...
    switch (buttonId) {
      case ToolbarItem.BACK:
        ...
        if (this.flagePopup === true) {
          this.router.navigate(["hrms/profile/list/reportconfig"]);
        }
        break;
  ...
  saveData = () => {
    ...
    if (this.flagState$.value === "new") {
      this._coreService.Post("package/report/add", param).subscribe(
        (res: any) => {
          //check error
      ...
      this._coreService.Post("package/report/Update", param).subscribe(
        (res: any) => {
          //check error
          if (res.statusCode == 400) {
            this.notification.checkErrorMessage(res.message);
  ...
  convertModel(param: any) {
    ...
    model.dateEffect = moment(model.dateEffect).format("YYYY-MM-DD");
    return model;
  }
  changeSalaryScale(e: any) {
    if (e.e) {
      this._coreService
        .Get("package/Salaryrank/GetList?scaleId=" + e.itemData.id)
        .subscribe((res: any) => {
