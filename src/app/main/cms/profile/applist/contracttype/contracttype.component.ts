...
import { ToolbarItem, ToolbarInterface, ContractType } from "src/app/_models/index";
...
export class ContractTypeComponent implements OnInit {
  ...
  viewRecord = (event: any) => {
    ...
    this.router.navigate(["/hrms/profile/list/contracttype/", paramAdd]);
  };

  // Build Toolbar
  buildToolbar = () => {
    const toolbarList = [ToolbarItem.ADD, ToolbarItem.EDIT, ToolbarItem.LOCK];
    this.toolbar = this.globals.buildToolbar("contracttype", toolbarList);
  };
  // filter checkbox
  ...
  getListData = (): void => {
    ...
    this._coreService.execute(this.state, "hr/contracttype/GetAll");
  };
  ...
  public dataStateChange(state: DataStateChangeEventArgs): void {
    ...
    let extraParams: any[] = [];
    this._coreService.execute(this.state, "hr/contracttype/GetAll", extraParams);
  }
  // Số thứ tự
  ...
  clickToolbar = (itemButton: any): void => {
    ...
    switch (buttonId) {
      case ToolbarItem.ADD:
        this.dialogHeader = "Thêm mới loại hợp đồng";
      ...
      case ToolbarItem.EXPORT_EXCEL:
        var data = this._coreService.ExecuteToExport('hr/contracttype/GetAll','',this.state).subscribe((res: any) => {
          // Lấy ngày hiện tại
          ...
          const exportProperties: ExcelExportProperties = {
            dataSource: res.data,
            columns: [],
            fileName: "loaihopdong" + "-" + formattedDate + ".xlsx"
          };
      ...
      case ToolbarItem.LOCK:
        if (selectDeletes && selectDeletes.length > 0) {
          ...
          this._coreService
            .Post("hr/contracttype/ChangeStatus", ids)
            .subscribe((res: any) => {
  ...
  confirmDelete = (status: any): void => {
    if (status === "cancel") {
      ...
      if (this.modelDelete.length > 0) {
        ...
        this._coreService
          .Post("hr/contracttype/delete", ids)
          .subscribe((res: any) => {
            if (res.statusCode == 200) {
  ...
  saveData = () => {
    ...
    if (this.flagState$.value === Consts.add) {
      this._coreService.Post("hr/contracttype/add", this.model).subscribe(
        (res: any) => {
      ...
      this._coreService.Post("hr/contracttype/Update", this.model).subscribe(
        (res: any) => {
          if (res.statusCode == 400) {
            this.notification.checkErrorMessage(res.message);
          } else {
