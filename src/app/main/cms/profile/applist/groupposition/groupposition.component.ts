...
export class GroupPositionComponent implements OnInit {
  ...
  // edit
  model: GroupPosition = new GroupPosition();
  modelTemp: GroupPosition = new GroupPosition();
  editForm!: FormGroup;
  ...
  viewRecord = (event: any) => {
    ...
    this.router.navigate(["/hrms/profile/list/groupposition/", paramAdd]);
  };
  ...
  getListData = (): void => {
    ...
    this._coreService.execute(this.state, "hr/groupposition/GetAll");
  };
  ...
  public dataStateChange(state: DataStateChangeEventArgs): void {
    ...
    let extraParams: any[] = [];
    this._coreService.execute(this.state, "hr/groupposition/GetAll", extraParams);
  }
  // Số thứ tự
  ...
  clickToolbar = (itemButton: any): void => {
    ...
    switch (buttonId) {
      ...
      case ToolbarItem.EXPORT_EXCEL:
        var data = this._coreService.ExecuteToExport('hr/groupposition/GetAll','',this.state).subscribe((res: any) => {
          // Lấy ngày hiện tại
      ...
      case ToolbarItem.LOCK:
        if (selectDeletes && selectDeletes.length > 0) {
          ...
          this._coreService
            .Post("hr/groupposition/ChangeStatus", ids)
            .subscribe((res: any) => {
  ...
  confirmDelete = (status: any): void => {
    if (status === "cancel") {
      ...
      if (this.modelDelete.length > 0) {
        ...
        this._coreService
          .Post("hr/groupposition/Delete", ids)
          .subscribe((res: any) => {
            if (res.statusCode == 200) {
  ...
  getById(id: any) {
    ...
    if (id) {
      this._coreService
        .Get("hr/groupposition/get?id=" + id)
        .subscribe((res: any) => {
  ...
  saveData = () => {
    ...
    if (this.flagState$.value === Consts.add) {
      this._coreService.Post("hr/groupposition/add", this.model).subscribe(
        (res: any) => {
      ...
      this._coreService.Post("hr/groupposition/Update", this.model).subscribe(
        (res: any) => {
          if (res.statusCode == 400) {
            this.notification.checkErrorMessage(res.message);
          } else {
