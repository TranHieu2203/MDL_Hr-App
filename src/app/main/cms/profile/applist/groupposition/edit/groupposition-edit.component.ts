...
import { GroupPosition } from "src/app/_models/app/cms/index";
...
export class GroupPositionEditComponent implements OnInit {
    ...
    async.waterfall(
      [
        (cb: any) => {
          if (this.paramId) {
            this._coreService
              .Get("hr/GroupPosition/get?id=" + this.paramId)
              .subscribe((res: any) => {
    ...
    switch (buttonId) {
      case ToolbarItem.BACK:
        ...
        if (this.flagePopup === true) {
          this.router.navigate(["hrms/profile/list/groupposition"]);
        }
        break;
  ...
  saveData = () => {
    ...
    if (this.flagState$.value === "new") {    
      this._coreService.Post("hr/groupposition/add", param).subscribe(
        (res: any) => {
      ...
      this._coreService.Post("hr/groupposition/Update", param).subscribe(
        (res: any) => {
