...
import { ToolbarItem, ToolbarInterface, InsRegimeType } from "src/app/_models/index";
...
@Component({
  selector: "cms-app-insregimetype",
  templateUrl: "./insregimetype.component.html",
  styleUrls: ["./insregimetype.component.scss"],
  providers: [FilterService, VirtualScrollService],
  encapsulation: ViewEncapsulation.None,
})
export class InsRegimeTypeComponent implements OnInit {
  ...
  // edit
  model: InsRegimeType = new InsRegimeType();
  modelTemp: InsRegimeType = new InsRegimeType();
  editForm!: FormGroup;
  ...
  constructor(
    ...
    this.editForm = this._formBuilder.group({
      ...
      nameEn: ["", [Validators.required, Validators.maxLength(51)]],
      code: [
        "",
        [
          Validators.required,
  ...
  viewRecord = (event: any) => {
    ...
    this.router.navigate(["/hrms/profile/list/insregimetype/", paramAdd]);
  };
  ...
  buildToolbar = () => {
    ...
    this.toolbar = this.globals.buildToolbar("insregimetype", toolbarList!);
  };
  // filter checkbox
  ...
  getListData = (): void => {
    ...
    this._coreService.execute(this.state, "hr/insregimetype/GetAll");
  };
  ...
  public dataStateChange(state: DataStateChangeEventArgs): void {
    ...
    let extraParams: any[] = [];
    this._coreService.execute(this.state, "hr/insregimetype/GetAll", extraParams);
  }
  // Số thứ tự
  ...
  clickToolbar = (itemButton: any): void => {
    ...
    switch (buttonId) {
      case ToolbarItem.ADD:
        this.dialogHeader = "Thêm mới chế độ bảo hiểm";
        this.flagState$.next(Consts.add)
        this.model = new InsRegimeType();
      ...
      case ToolbarItem.EXPORT_EXCEL:
        var data = this._coreService.ExecuteToExport('hr/insregimetype/GetAll','',this.state).subscribe((res: any) => {
          // Lấy ngày hiện tại
          ...
          const exportProperties: ExcelExportProperties = {
            dataSource: res.data,
            columns: [],
            fileName: "chedobaohiem" + "-" + formattedDate + ".xlsx"
          };
      ...
      case ToolbarItem.LOCK:
        if (selectDeletes && selectDeletes.length > 0) {
          ...
          this._coreService
            .Post("hr/insregimetype/ChangeStatus", ids)
            .subscribe((res: any) => {
  ...
  confirmDelete = (status: any): void => {
    if (status === "cancel") {
      ...
      if (lstDeleteIds.length > 0) {
        this._coreService
          .Post("hr/insregimetype/delete", lstDeleteIds)
  ...
  getById(id: any) {
    ...
    if (id) {
      this._coreService
        .Get("hr/insregimetype/get?id=" + id)
        .subscribe((res: any) => {
  ...
  saveData = () => {
    ...
    if (this.flagState$.value === Consts.add) {
      this._coreService.Post("hr/insregimetype/add", this.model).subscribe(
        (res: any) => {
      ...
      this._coreService.Post("hr/insregimetype/Update", this.model).subscribe(
        (res: any) => {
          if (res.statusCode == 400) {
            this.notification.checkErrorMessage(res.message);
          } else {
