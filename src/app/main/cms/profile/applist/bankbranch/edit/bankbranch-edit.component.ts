...
import { BankBranch } from "src/app/_models/app/cms/index";
...
export class BankBranchEditComponent implements OnInit {
    ...
    async.waterfall(
      [
        (cb: any) => {
          if (this.paramId) {
            this._coreService
              .Get("hr/bankbranch/get?id=" + this.paramId)
              .subscribe((res: any) => {
        ...
        (cb: any) => {
          this._coreService.Get("hr/bank/GetList").subscribe((res: any) => {
    ...
    switch (buttonId) {
      case ToolbarItem.BACK:
        ...
        if (this.flagePopup === true) {
          this.router.navigate(["hrms/profile/list/bankbranch"]);
        }
        break;
  ...
  saveData = () => {
    ...
    if (this.flagState$.value === "new") {    
      this._coreService.Post("hr/bankbranch/add", param).subscribe(
        (res: any) => {
          //check error
      ...
      this._coreService.Post("hr/bankbranch/Update", param).subscribe(
        (res: any) => {
          //check error
          if (res.statusCode == 400) {
            this.notification.checkErrorMessage(res.message);
