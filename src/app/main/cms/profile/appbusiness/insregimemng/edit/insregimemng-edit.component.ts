...
import { InsRegimeMng } from "src/app/_models/app/cms";
setCulture("en");

@Component({
  selector: "cms-profile-insregimemng-edit",
  templateUrl: "./insregimemng-edit.component.html",
  styleUrls: ["./insregimemng-edit.component.scss"],
  providers: [FilterService, VirtualScrollService],
  encapsulation: ViewEncapsulation.None,
})
export class InsRegimeMngEditComponent implements OnInit {
  ...
  model: InsRegimeMng = new InsRegimeMng();
  modelTemp: InsRegimeMng = new InsRegimeMng();
  modelTemp1: InsRegimeMng = new InsRegimeMng();
  languages: any;
  ...
  constructor(
    ...
    this.editForm = this._formBuilder.group({
      ...
      positionName: [""],
      orgName: [""],
      regimeTypeId: ["", [Validators.required]],
      regimeTypeName: ["",[]] ,
  ...
  loadData() {
    Promise.all([
      new Promise((resolve) => {
        if (this.paramId) {
          this._coreService
            .Get("hr/insregimemng/get?id=" + this.paramId)
            .subscribe((res: any) => {
              resolve(res.data);
            });
        } else {
      ...
      new Promise((resolve) => {
        this._coreService.Get("hr/insregimetype/GetList").subscribe((res: any) => {
          resolve(res.data);
        });
      }),
      new Promise((resolve) => {
        this._coreService.Get("hr/otherlist/PAYMENT").subscribe((res: any) => {
          resolve(res.data);
        });
      }),
    ]).then((res: any[]) => {
      this.lstregimeTypeId = res[1];
      this.lstpayType = res[2];
      if (this.paramId) {
  ...
  choiseEmp() {
    if (this.flagState$.value == "view" || this.flagState$.value == "edit") {
      return;
    }
    let param = {
    ...
    switch (buttonId) {
      case ToolbarItem.BACK:
        ...
        if (this.flagePopup === true) {
          this.router.navigate(["hrms/profile/business/insregimemng"]);
        }
        break;
  ...
  saveData = () => {
    ...
    if (this.flagState$.value === "new") {
      this._coreService.Post("hr/insregimemng/add", param).subscribe(
        (res: any) => {
      ...
      this._coreService.Post("hr/insregimemng/Update", param).subscribe(
        (res: any) => {
          if (res.statusCode == 400) {
            this.notification.checkErrorMessage(res.message);
          } else {
  ...
  convertModel(param: any) {
    ...
    model.endDate = param.endDate
      ...
      : null;
    model.periodDate = param.periodDate
      ? moment(param.periodDate).format("YYYY-MM-DD")
      : null;
    return model;
  }
  calculate (){
    if(this.model.employeeId != undefined  && this.model.regimeTypeId != undefined && this.model.dayCount != undefined){
      let param = this.convertModel(this.model);
      this._coreService.Post("hr/insregimemng/Calculate", param).subscribe(
        (res: any) => {
          ...
          if (res.statusCode == 400) {
            ...
            this.model.salIns = res.data.salIns;
            this.model.salAllowance = res.data.salAllowance;
          }
        },
        (error: any) => {
          this.notification.editError();
    ...
    if(this.model.startDate != undefined && this.model.endDate != undefined  && this.model.regimeTypeId != undefined && this.model.dayCount ==undefined){
      let param = this.convertModel(this.model);
      this._coreService.Post("hr/insregimemng/CalculateDayCount", param).subscribe(
        (res: any) => {
          ...
          if (res.statusCode == 400) {
            ...
            this.model.dayCount = res.data.dayCount;
            if(this.model.employeeId != undefined  && this.model.regimeTypeId != undefined && this.model.dayCount != undefined){
              let param = this.convertModel(this.model);
              this._coreService.Post("hr/insregimemng/Calculate", param).subscribe(
                (res: any) => {
  ...
  changeDateNoRequire1 = (model: any) => {
    setTimeout(() => {
      const idDate = "#" + model + "_input";
      const value = $(idDate).val();
      var patt = new RegExp(
