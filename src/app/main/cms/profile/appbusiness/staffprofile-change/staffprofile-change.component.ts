...
import { ToolbarItem, ToolbarInterface, Employee } from "src/app/_models/index";
...
export class StaffProfileChangeComponent implements OnInit, OnDestroy {
  ...
  viewRecord = (event: any) => {
    ...
    const objParamAdd = { id: this.modelAdd.id, employeeId: this.modelAdd.employeeId, type: "view" };
    const paramAdd = window.btoa(JSON.stringify(objParamAdd));
    this.router.navigate(["/hrms/profile/business/staffprofile-change/", paramAdd]);
  ...
  clickToolbar = (itemButton: any): void => {
    ...
    switch (buttonId) {
      ...
      case ToolbarItem.EXPORT_EXCEL:
        ...
        var data = this._coreService.ExecuteToExport('hr/Employee/GetAllProfileEdit', extraParams, this.state).subscribe((res: any) => {
          // Lấy ngày hiện tại
          ...
          const exportProperties: ExcelExportProperties = {
            dataSource: res.data,
            columns: [],
            fileName: "pheduyetthongtinSYLL" + "-" + formattedDate + ".xlsx"
          };
  ...
  confirmApproved = (status: any): void => {
    if (status === "cancel") {
      ...
      let lstDeleteIds = _.map(this.modelDelete, "statusId");
