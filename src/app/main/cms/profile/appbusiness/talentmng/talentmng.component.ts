...
@Component({
  selector: "cms-app-talentmng",
  templateUrl: "./talentmng.component.html",
  styleUrls: ["./talentmng.component.scss"],
  providers: [FilterService, VirtualScrollService],
  encapsulation: ViewEncapsulation.None,
})
export class TalentMngComponent implements OnInit {
  toolItems$ = new BehaviorSubject<any[]>([ToolbarItem.EXPORT_EXCEL]);
  // Varriable Language
  languages: any;
  ...
  viewRecord = (event: any) => {
    ...
    const objParamAdd = { id: this.modelAdd.employeeId, type: "view" };
    const paramAdd = window.btoa(JSON.stringify(objParamAdd));
    this.router.navigate(["/hrms/profile/business/talentmng/", paramAdd]);
  };

  // Build Toolbar
  buildToolbar = () => {
    const toolbarList = [null];
    this.toolbar = this.globals.buildToolbar("talentmng", toolbarList!);
  };
  // filter checkbox
  ...
  getListData = (): void => {
    ...
    this._coreService.execute(this.state, "hr/talentsetting/GetAllMng");
  };
  ...
  public dataStateChange(state: DataStateChangeEventArgs): void {
    ...
    let extraParams: any[] = [];
    this._coreService.execute(this.state, "hr/talentsetting/GetAllMng", extraParams);
  }
  // Số thứ tự
  ...
  clickToolbar = (itemButton: any): void => {
    ...
    switch (buttonId) {
      case ToolbarItem.ADD:
        this.router.navigate(["/hrms/profile/business/talentsetting/new"]);
        break;
      case ToolbarItem.EDIT:
        ...
        if (selectRows && selectRows.length > 0) {
          ...
          this.router.navigate([
            "/hrms/profile/business/talentsetting/",
            paramAdd,
          ]);
        } else {
      ...
      case ToolbarItem.EXPORT_EXCEL:
        var data = this._coreService.ExecuteToExport('hr/talentsetting/GetAllMng').subscribe((res: any) => {
          // Lấy ngày hiện tại
          ...
          const exportProperties: ExcelExportProperties = {
            dataSource: res.data,
            columns: [],
            fileName: "hosonhantai" + "-" + formattedDate + ".xlsx"
          };
      ...
      case ToolbarItem.LOCK:
        if (selectDeletes && selectDeletes.length > 0) {
          ...
          this._coreService
            .Post("hr/talentsetting/ChangeStatus", ids)
            .subscribe((res: any) => {
  ...
  confirmDelete = (status: any): void => {
    if (status === "cancel") {
      ...
      if (lstDeleteIds.length > 0) {
        this._coreService
          .Post("hr/talentsetting/Delete", ids)
