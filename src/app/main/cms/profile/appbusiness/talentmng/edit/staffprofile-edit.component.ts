...
export class StaffProfileEditComponent implements OnInit {
  ...
  loadData() {
    Promise.all([
      ...
      if (this.paramId) {
        this.model = _.cloneDeep(_.omit(res[0], ["districtId", "wardId","positionId","directManagerId","directManagerName","directManagerTitleName"], ["curDistrictId", "curWardId"]));
    ...
    switch (buttonId) {
      case ToolbarItem.BACK:
        ...
        if (this.flagePopup === true) {
          this.router.navigate(["hrms/profile/business/talentmng"]);
        }
        break;
  ...
  ViewHistory(param: any) {
    ...
    if (param == "workingbefore") {
      this._coreService
        .Get("hr/workingbefore/GetAll?PageNo=1&PageSize=500&orgId=" + this.model.orgId + "&employeeCode=" + this.model.code)
        .subscribe((res: any) => {
          this.dataHistory = res.data
        });
    }
    if (param == "trainingbefore") {
      this._coreService
        .Get("hr/trainingbefore/GetAll?PageNo=1&PageSize=500&orgId=" + this.model.orgId + "&employeeCode=" + this.model.code)
        .subscribe((res: any) => {
          this.dataHistory = res.data
        });
    }
