...
export class LeaveJobComponent implements OnInit {
  ...
  viewRecord = (event: any) => {
    ...
    this.router.navigate(["/hrms/profile/business/leavejob/", paramAdd]);
  };
  ...
  getListData = (): void => {
    ...
    this._coreService.execute(this.state, "hr/terminate/GetAll", extraParams);
  };
  ...
  clickToolbar = (itemButton: any): void => {
    ...
    switch (buttonId) {
      case ToolbarItem.ADD:
        this.router.navigate(["/hrms/profile/business/leavejob/new"]);
        break;
      ...
      case ToolbarItem.PRINT:
        if (selectDeletes && selectDeletes.length > 0) {
          ...
          let typeid = JSON.stringify(selectDeletes.map((i: any) => i.contractTypeId));
          let paramExport = {
            sid: JSON.stringify(ids),
            typeId: typeid
          }
          this._coreService
            .templateImportV2Hrm("PrintTerminate", paramExport)
            .subscribe((res: any) => {
          ...
          /* 
           this._coreService
             .Get("hr/FormList/PrintForm?typeId=7&id=" + ids[0])
             .subscribe((res: any) => {
               //check error
      ...
      case ToolbarItem.EXPORT_EXCEL:
        ...
        var data = this._coreService.ExecuteToExport('hr/terminate/GetAll', extraParams, this.state).subscribe((res: any) => {
          // Lấy ngày hiện tại
          ...
          const exportProperties: ExcelExportProperties = {
            dataSource: res.data,
            columns: [],
            fileName: "nghiviec" + "-" + formattedDate + ".xlsx"
          };
      ...
      case ToolbarItem.APPROVED:
        if (selectDeletes && selectDeletes.length > 0) {
          ...
          this._coreService
            .Post("hr/terminate/approved", ids[0])
            .subscribe((res: any) => {
              if (res.statusCode == 400) {
                this.notification.checkErrorMessage(res.message);
              } else {
      ...
      case ToolbarItem.SYNC:
        if (selectDeletes && selectDeletes.length > 0) {
          ...
          if (lstDeleteIds.length > 0) {
            this._coreService
              .Post("hr/terminate/PushErpTerminate", lstDeleteIds)
