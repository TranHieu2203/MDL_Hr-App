...
import { ToolbarItem, ToolbarInterface, WorkingBefore } from "src/app/_models/index";
...
@Component({
  selector: "cms-profile-workingbefore-change",
  templateUrl: "./workingbefore-change.component.html",
  styleUrls: ["./workingbefore-change.component.scss"],
  providers: [FilterService],
  encapsulation: ViewEncapsulation.None,
})
export class WorkingBeforeChangeComponent implements OnInit, OnDestroy {
  ...
  viewRecord = (event: any) => {
    ...
    const objParamAdd = { id: this.modelAdd.id, idWorking: this.modelAdd.idWorking, type: "view" };
    const paramAdd = window.btoa(JSON.stringify(objParamAdd));
    this.router.navigate(["/hrms/profile/business/workingbefore-change/", paramAdd]);
  };
  ...
  getListData = (): void => {
    ...
    this._coreService.execute(this.state, "hr/Employee/GetAllWorkingBeforeAdd", extraParams);
  };
  GetEmp = (e: any) => {
  ...
  clickToolbar = (itemButton: any): void => {
    ...
    switch (buttonId) {
      ...
      case ToolbarItem.EXPORT_EXCEL:
        ...
        var data = this._coreService.ExecuteToExport('hr/Employee/GetAllWorkingBeforeAdd', extraParams, this.state).subscribe((res: any) => {
          // Lấy ngày hiện tại
          ...
          const exportProperties: ExcelExportProperties = {
            dataSource: res.data,
            columns: [],
            fileName: "pheduyetquatrinhcongtactruocday" + "-" + formattedDate + ".xlsx"
          };
  ...
  confirmApproved = (status: any): void => {
    if (status === "cancel") {
      ...
      if (lstDeleteInfo.length > 0) {
        this._coreService
          .Post("hr/Employee/ApproveWorkingEdit", lstDeleteInfo)
  ...
  confirmUnApproved = (status: any): void => {
    if (status === "cancel") {
      ...
      if (lstDeleteInfo.length > 0) {
        this._coreService
          .Post("hr/Employee/RejectWorkingEdit", lstDeleteInfo)
          .subscribe((success: any) => {
            if (success.statusCode == "200") {
