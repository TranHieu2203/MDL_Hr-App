...
import { Employee, Situation, PosPage, ParamGenCode } from "src/app/_models/app/cms";
...
@Component({
  selector: "cms-profile-staffprofile-edit",
  templateUrl: "./staffprofile-edit.component.html",
  styleUrls: ["./staffprofile-edit.component.scss"],
...
export class StaffProfileEditComponent implements OnInit {
  ...
  constructor(
    ...
    this.editForm = this._formBuilder.group({
      currentinfor: this._formBuilder.group({
        ...
        itimeCode: ["", [Validators.required]], //Mã chấp công
        orgManager: ["", []], //
        positionId: ["", [Validators.required]], //chức vụ
        staffRankId: [""],
        directManagerName: ["", []],
        directManagerTitleName: ["", []],
        taxCode: [""], //Mã sô thuế
        seniorityText: [""],
        salTotal: ["", []],
        lstPeriod: [""],
        contractCode: [{ value: null, disabled: true }, []], //Loại hợp đồng
        classifyName: [{ value: null, disabled: true }, []], //Phân loại
  ...
  ngOnInit(): void {
    ...
    this.flagState$.subscribe(x => {
      ...
      if (x === "edit") {
        ...
        var JoinDateStatus = xx.get('joinDate')
        if (this.model.contractId != null) {
          OrgIdStatus!.disable();
          JoinDateStatus!.disable();
  ...
  loadEmployeeCode() {
    ...
    this.modelAutoGen.dEffectDate = (moment(new Date()).format("DD/MM/YYYY"));
    this.modelAutoGen.TypeId = 0;
  ...
  loadData() {
    Promise.all([
      ...
      if (this.paramId) {
        this.model = _.cloneDeep(_.omit(res[0], ["districtId", "wardId", "positionId", "directManagerId", "directManagerName", "directManagerTitleName", "titleName", "groupTitleName"], ["curDistrictId", "curWardId"], ["homeDistrictId", "homeWardId"], ["bankBranch"]));
  ...
  loadDatalazy(model: Employee) {
    ...
    if (model && model.orgId) {
      this.getPosition(model.orgId, model.id)
        ...
        .then((x) => {
          ...
          this.model.titleName = model.titleName;
          this.model.groupTitleName = model.groupTitleName;
    ...
    if (model && model.avatar != undefined) {
      // var filename = model.avatar.replace("FileManager/profile/avatar/","");
      // this._coreService.binaryImage("", Consts.profile, "avatar",filename).subscribe((res: any) => {
      //   this.fileAvatar = "data:image/jpeg;base64," + res.data;
      // });
      this._coreService.commonGetFile("File/employee-image-by-url?url=" + model.avatar).subscribe((data: any) => {
            
        if (data.body.status) {
          this.fileAvatar = data.body.data
        } else {
    ...
    switch (buttonId) {
      ...
      case ToolbarItem.SAVE:
        if (this.tab == 5) {
          ...
          if (this.model.taxCode != undefined) {
            if (this.model.taxCode.length != 10) {
              this.notification.warning("Mã số thuế cá nhân phải là 10 chữ số!")
              return;
            }
          }

          this.saveData();
          // this.flagState$.next("view");
        }
        break;
  ...
  choiseOrg() {
    ...
    if (this.flagState$.value == "edit") {
      ...
      //   return;
      // }
      if (this.model.lastWorkingId != null) {
        this.notification.warning('Hồ sơ đã tồn tại quyết định, bạn không thể thay đổi đơn vị !');
        return;
      }
    }
  ...
  uploadAvatar(files: FileList | null) {
    setTimeout(() => {
      if (files!.length > 0) {
        ...
        data.append("files", "profile");
        data.append("files", files![0]);
        this._coreService.uploadFileV2Hrm(data, Consts.profile, "avatar").subscribe((res: any) => {
          var url = this.globals.apiUrlFileManager.toString().replace("api/", "") + res.data;
          var url1 = res.data;
          this.model.avatar = url1;
          // this.fileAvatar = url;
          let x: any = document.getElementById("avatar");
          x.value = null;
          
          // var filename = url1.replace("FileManager/profile/avatar/","");
          
          // if(filename!= null){
          //   this._coreService.binaryImage(data, Consts.profile, "avatar",filename).subscribe((res: any) => {
          //     this.fileAvatar = "data:image/jpeg;base64," + res.data;
          //   });
          // }
          this._coreService.commonGetFile("File/employee-image-by-url?url=" + this.model.avatar).subscribe((data: any) => {
  ...
  RemoveRelation2(id: any) {
    this.removeId = id;
    this.modalService.open("confirm-back-modal2");
    // this.modalService.open("confirm-delete-modal1");
  }
  ...
  confirmDeletePaper = (status: any): void => {
    if (status === "cancel") {
      this.modalService.close("confirm-back-modal2");
    } else {
      this._coreService.Post("hr/employee/RemovePaper", this.removeId).subscribe(
        (res: any) => {
          //check error
          if (res.statusCode == 400) {
            this.notification.checkErrorMessage(res.message);
  ...
  getListSituation() {
    this._coreService
      .Get("hr/Employee/ListSituationProfile?empId=" + this.model.id)
      .subscribe((res: any) => {
        this.data = res.data;
  ...
  changeIdNo() {
    if (this.model.idNo != null) {
      ...
      this._coreService
        .Get("hr/Employee/GetByIdNo?id=" + this.model.idNo)
        .subscribe((res: any) => {
