...
@Component({
  selector: "cms-profile-insarising",
  templateUrl: "./insarising.component.html",
  styleUrls: ["./insarising.component.scss"],
  providers: [FilterService, VirtualScrollService],
  encapsulation: ViewEncapsulation.None,
})
export class InsArisingComponent implements OnInit {
  ...
  model: InsArising = new InsArising();
  public modelAdd: any;
  ...
  getListData = (): void => {
    ...
    this._coreService.execute(this.state, "hr/insarising/GetAll", extraParams);
  };
  ...
  clickToolbar = (itemButton: any): void => {
    ...
    switch (buttonId) {
      case ToolbarItem.ADD:
        ...
        this.router.navigate(["/hrms/profile/business/insarising/new"]);
        break;
      case ToolbarItem.EDIT:
        ...
        if (selectRows && selectRows.length > 0) {
          ...
          const paramAdd = window.btoa(JSON.stringify(objParamAdd));
          this.router.navigate(["/hrms/profile/business/insarising/", paramAdd]);
        } else {
          this.notification.warning("notify.NO_RECORD_SELECT");
        }
        break;
      case ToolbarItem.SAVE:
        const selectRows1 = this.gridInstance.getSelectedRecords();
        if (selectRows1 && selectRows1.length > 0) {
          this.modelAdd = selectRows1[0];
          if (this.model.changeMonth != null) {
            this.modelAdd.changeMonth = this.model.changeMonth;
          } else {
            this.notification.warning("Chưa chọn tháng biến động!");
            return;
          }

          let param = this.convertModel(this.modelAdd);

          this._coreService.Post("hr/insarising/save", param).subscribe(
            (res: any) => {
      ...
      case ToolbarItem.EXPORT_EXCEL:
        ...
        var data = this._coreService.ExecuteToExport('hr/insarising/GetAll', extraParams, this.state).subscribe((res: any) => {
          // Lấy ngày hiện tại
          ...
          const exportProperties: ExcelExportProperties = {
            dataSource: res.data,
            columns: [],
            fileName: "khaibaodongmoi" + "-" + formattedDate + ".xlsx"
          };
      ...
      case ToolbarItem.LOCK:
        if (selectDeletes && selectDeletes.length > 0) {
          ...
          this._coreService
            .Post("hr/insarising/ChangeStatus", ids)
            .subscribe((res: any) => {
  ...
  confirmDelete = (status: any): void => {
    if (status === "cancel") {
      ...
      if (lstDeleteIds.length > 0) {
        this._coreService
          .Post("hr/insarising/delete", lstDeleteIds)
  ...
  convertModel(param: any) {
    ...
    model.changeMonth = param.changeMonth
      ? moment(param.changeMonth).format("YYYY-MM-DD")
      : null;
    return model;
  }
  ngOnDestroy() {
