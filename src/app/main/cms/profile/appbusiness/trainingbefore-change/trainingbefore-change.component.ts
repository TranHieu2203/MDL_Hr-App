...
import { ToolbarItem, ToolbarInterface, TrainingBefore } from "src/app/_models/index";
...
@Component({
  selector: "cms-profile-trainingbefore-change",
  templateUrl: "./trainingbefore-change.component.html",
  styleUrls: ["./trainingbefore-change.component.scss"],
  providers: [FilterService],
  encapsulation: ViewEncapsulation.None,
})
export class TrainingBeforeChangeComponent implements OnInit, OnDestroy {
  ...
  viewRecord = (event: any) => {
    ...
    const objParamAdd = { id: this.modelAdd.id, idTraining: this.modelAdd.idTraining, type: "view" };
    const paramAdd = window.btoa(JSON.stringify(objParamAdd));
    this.router.navigate(["/hrms/profile/business/trainingbefore-change/", paramAdd]);
  };
  ...
  getListData = (): void => {
    ...
    this._coreService.execute(this.state, "hr/Employee/GetAllTrainingBeforeAdd", extraParams);
  };
  GetEmp = (e: any) => {
  ...
  clickToolbar = (itemButton: any): void => {
    ...
    switch (buttonId) {
      ...
      case ToolbarItem.EXPORT_EXCEL:
        ...
        var data = this._coreService.ExecuteToExport('hr/Employee/GetAllTrainingBeforeAdd', extraParams, this.state).subscribe((res: any) => {
          // Lấy ngày hiện tại
          ...
          const exportProperties: ExcelExportProperties = {
            dataSource: res.data,
            columns: [],
            fileName: "pheduyetquatrinhdaotaotruocday" + "-" + formattedDate + ".xlsx"
          };
  ...
  confirmApproved = (status: any): void => {
    if (status === "cancel") {
      ...
      if (lstDeleteInfo.length > 0) {
        this._coreService
          .Post("hr/Employee/ApproveTrainingEdit", lstDeleteInfo)
  ...
  confirmUnApproved = (status: any): void => {
    if (status === "cancel") {
      ...
      if (lstDeleteInfo.length > 0) {
        this._coreService
          .Post("hr/Employee/RejectTrainingEdit", lstDeleteInfo)
          .subscribe((success: any) => {
            if (success.statusCode == "200") {
