...
export class DecisionCreateComponent implements OnInit {
  ...
  viewRecord = (event: any) => {
    ...
    this.router.navigate(["/hrms/profile/setting/decisioncreate/", paramAdd]);
    this.router.navigate(["/hrms/profile/setting/decisioncreate/", paramAdd]);
  };
  ...
  getListData = (): void => {
    ...
    this._coreService.execute(this.state, "hr/sysdecision/getallbydecision");
  };
  ...
  public dataStateChange(state: DataStateChangeEventArgs): void {
    ...
    this._coreService.execute(
      state,
      "hr/sysdecision/getallbydecision",
      extraParams
    );
  }
  // Số thứ tự
  ...
  clickToolbar = (itemButton: any): void => {
    ...
    switch (buttonId) {
      case ToolbarItem.ADD:
        this.router.navigate(["/hrms/profile/setting/decisioncreate/new"]);
        break;
      case ToolbarItem.EDIT:
        ...
        if (selectRows && selectRows.length > 0) {
          ...
          this.router.navigate([
            "/hrms/profile/setting/decisioncreate/",
            paramAdd,
          ]);
        } else {
  ...
  confirmDelete = (status: any): void => {
    if (status === "cancel") {
      ...
      if (this.modelDelete.length > 0) {
        ...
        this._coreService
          .Post("hr/sysdecision/remove", ids)
          .subscribe((res: any) => {
            if (res.statusCode == 200) {
