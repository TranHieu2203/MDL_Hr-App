...
import { SearchModel,UpdateStatusCvModel } from "src/app/_models/app/cms/common/searchmodel";
...
@Component({
  selector: "cms-recruitment-candidate",
...
export class CandidateComponent implements OnInit, OnDestroy {
  ...
  constructor(
    ...
    this.model1 = new UpdateStatusCvModel();
  }
  ...
  viewRecord = (event: any) => {
    ...
    this.router.navigate(["/hrms/recruitment/business/candidate/", paramAdd]);
  };
  ...
  getListData = (): void => {
    ...
    this._coreService.execute(this.state, "hr/candidate/GetAll", extraParams);
  };
  ...
  clickToolbar = (itemButton: any): void => {
    ...
    switch (buttonId) {
      case ToolbarItem.ADD:
        this.router.navigate(["/hrms/recruitment/business/candidate/new"]);
        break;
      case ToolbarItem.EDIT:
        ...
        if (selectRows && selectRows.length > 0) {
          ...
          this.router.navigate([
            "/hrms/recruitment/business/candidate/",
            paramAdd,
          ]);
        } else {
      ...
      case ToolbarItem.DELETE:
        ...
        break;
        case ToolbarItem.UPDATESTATUSCV:
          if (selectDeletes && selectDeletes.length > 0) {
            if(this.model1.statusCv == undefined){
              this.notification.warning("Chưa chọn trạng thái ứng viên");
              return;
            }
            this.modelDelete = selectDeletes;
            let lstDeleteIds = _.map(this.modelDelete, "id");
            this.modelTemp.emps = lstDeleteIds;
            this.modelTemp.statusCv = this.model1.statusCv;
            if (lstDeleteIds.length > 0) {
              this._coreService
                .Post("hr/candidate/UpdateStatusCv", this.modelTemp)
                .subscribe((success: any) => {
                  if (success.statusCode == "200") {
                    this.notification.editSuccess();
                    this.gridInstance.clearSelection();
                    this.gridInstance.refresh();
                    this.model1.statusCv = undefined;
                  }
                  else {
      ...
      var data = this._coreService.ExecuteToExport('hr/candidate/GetAll', extraParams,this.state).subscribe((res: any) => {
        // Lấy ngày hiện tại
        ...
        const exportProperties: ExcelExportProperties = {
          dataSource: res.data,
          columns: [],
          fileName: "quanlyungvien" + "-" + formattedDate +".xlsx"
        };
