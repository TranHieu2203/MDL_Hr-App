...
import { ToolbarItem, ToolbarInterface, GroupCriteria } from "src/app/_models/index";
...
@Component({
  selector: "cms-app-groupcriteria",
  templateUrl: "./groupcriteria.component.html",
  styleUrls: ["./groupcriteria.component.scss"],
  providers: [FilterService, VirtualScrollService],
  encapsulation: ViewEncapsulation.None,
})
export class GroupCriteriaComponent implements OnInit {
  ...
  // edit
  model: GroupCriteria = new GroupCriteria();
  modelTemp: GroupCriteria = new GroupCriteria();
  editForm!: FormGroup;
  ...
  viewRecord = (event: any) => {
    ...
    this.router.navigate(["/hrms/profile/list/GroupCriteria/", paramAdd]);
  };
  ...
  getListData = (): void => {
    ...
    this._coreService.execute(this.state, "hr/GroupCriteria/GetAll");
  };
  ...
  public dataStateChange(state: DataStateChangeEventArgs): void {
    ...
    let extraParams: any[] = [];
    this._coreService.execute(this.state, "hr/GroupCriteria/GetAll", extraParams);
  }
  // Số thứ tự
  ...
  clickToolbar = (itemButton: any): void => {
    ...
    switch (buttonId) {
      case ToolbarItem.ADD:
        this.dialogHeader = "Thêm mới nhóm chức danh";
        this.flagState$.next(Consts.add)
        this.model = new GroupCriteria();
      ...
      case ToolbarItem.EXPORT_EXCEL:
        var data = this._coreService.ExecuteToExport('hr/GroupCriteria/GetAll','',this.state).subscribe((res: any) => {
          // Lấy ngày hiện tại
          ...
          const exportProperties: ExcelExportProperties = {
            dataSource: res.data,
            columns: [],
            fileName: "nhomchucdanh" + "-" + formattedDate + ".xlsx"
          };
      ...
      case ToolbarItem.LOCK:
        if (selectDeletes && selectDeletes.length > 0) {
          ...
          this._coreService
            .Post("hr/GroupCriteria/ChangeStatus", ids)
            .subscribe((res: any) => {
  ...
  confirmDelete = (status: any): void => {
    if (status === "cancel") {
      ...
      if (this.modelDelete.length > 0) {
        ...
        this._coreService
          .Post("hr/GroupCriteria/Delete", ids)
          .subscribe((res: any) => {
            if (res.statusCode == 200) {
  ...
  getById(id: any) {
    ...
    if (id) {
      this._coreService
        .Get("hr/GroupCriteria/get?id=" + id)
        .subscribe((res: any) => {
  ...
  saveData = () => {
    ...
    if (this.flagState$.value === Consts.add) {
      this._coreService.Post("hr/GroupCriteria/add", this.model).subscribe(
        (res: any) => {
      ...
      this._coreService.Post("hr/GroupCriteria/Update", this.model).subscribe(
        (res: any) => {
          if (res.statusCode == 400) {
            this.notification.checkErrorMessage(res.message);
          } else {
