...
import { ToolbarItem, ToolbarInterface, Criteria } from "src/app/_models/index";
...
export class CriteriaComponent implements OnInit {
  ...
  modelTemp: Criteria = new Criteria();
  editForm!: FormGroup;
  flagState$ = new BehaviorSubject<string>('');
  lstgroupCriteria: any;
  ...
  viewRecord = (event: any) => {
    ...
    this.router.navigate(["/hrms/organize/list/criteria/", paramAdd]);
  };

  // GetListData
  getListData = (): void => {
    
    this._coreService.execute(this.state, "hr/criteria/GetAll");
  };
  ...
  public dataStateChange(state: DataStateChangeEventArgs): void {
    ...
    let extraParams: any[] = [];
    this._coreService.execute(this.state, "hr/criteria/GetAll", extraParams);
  }
  // Số thứ tự
  ...
  clickToolbar = (itemButton: any): void => {
    ...
    switch (buttonId) {
      case ToolbarItem.ADD:
        this.dialogHeader = "Thêm mới chức danh";
        this.flagState$.next(Consts.add)
        this.model = new Criteria();
        this.getlstgroupCriteria();
      ...
      case ToolbarItem.EXPORT_EXCEL:
        var data = this._coreService.ExecuteToExport('hr/criteria/GetAll','',this.state).subscribe((res: any) => {
          // Lấy ngày hiện tại
          ...
          const exportProperties: ExcelExportProperties = {
            dataSource: res.data,
            columns: [],
            fileName: "chucdanh" + "-" + formattedDate + ".xlsx"
          };
      ...
      case ToolbarItem.LOCK:
        if (selectDeletes && selectDeletes.length > 0) {
          ...
          this._coreService
            .Post("hr/criteria/ChangeStatus", ids)
            .subscribe((res: any) => {
  ...
  confirmDelete = (status: any): void => {
    if (status === "cancel") {
      ...
      if (lstDeleteIds.length > 0) {
        this._coreService
          .Post("hr/criteria/Delete", lstDeleteIds)
  ...
  getById(id: any) {
    ...
    if (id) {
      this._coreService
        .Get("hr/criteria/get?id=" + id)
        .subscribe((res: any) => {

          this.modelTemp = res.data;
          this.model = _.cloneDeep(this.modelTemp);
          this.dialogHeader = "Sửa " + this.model.nameVn;
  ...
  getlstgroupCriteria() {
    ...
    this._coreService
      .Get("hr/groupcriteria/GetList")
      .subscribe((res: any) => {
        this.lstgroupCriteria = res.data;
      });
  ...
  saveData = () => {
    ...
    if (this.flagState$.value === Consts.add) {
      this._coreService.Post("hr/criteria/add", this.model).subscribe(
        (res: any) => {
      ...
      this._coreService.Post("hr/criteria/Update", this.model).subscribe(
        (res: any) => {
          if (res.statusCode == 400) {
            this.notification.checkErrorMessage(res.message);
          } else {
