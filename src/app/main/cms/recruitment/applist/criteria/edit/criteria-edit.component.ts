...
import { Criteria } from "src/app/_models/app/cms/index";
...
@Component({
  ...
  styleUrls: ["./criteria-edit.component.scss"],
...
export class CriteriaEditComponent implements OnInit {
  ...
  loadData() {
    if (this.paramId) {
      this._coreService
        .Get("hr/criteria/get?id=" + this.paramId)
        .subscribe((res: any) => {
          this.model = res.data;
        });
    } else{
      this._coreService
      .Get("hr/criteria/GetList")
      .subscribe((res: any) => {
    ...
    switch (buttonId) {
      case ToolbarItem.BACK:
        ...
        if (this.flagePopup === true) {
          this.router.navigate(["cms/organize/list/criteria"]);
        }
        break;
      ...
      this._coreService.Post("hr/criteria/add", this.model).subscribe(
        (res: any) => {
          ...
          if (res.statusCode == 400) {
            ...
            this.router.navigate(["/hrms/organize/list/criteria"]);
          }
        },
        (error: any) => {
  ...
  confirmBack = (status: any): void => {
    if (status === "cancel") {
      ...
      this.modalService.close("confirm-back-modal");
      this.router.navigate(["/hrms/recruitment/list/criteria"]);
    }
  };
}
