...
import { ToolbarItem, ToolbarInterface, Evaluation } from "src/app/_models/index";
...
export class EvaluationComponent implements OnInit {
  ...
  modelTemp: Evaluation = new Evaluation();
  editForm!: FormGroup;
  ...
  viewRecord = (event: any) => {
    ...
    this.router.navigate(["/hrms/recruitment/list/evaluation/", paramAdd]);
  };

  // GetListData
  getListData = (): void => {

    this._coreService.execute(this.state, "hr/evaluation/GetAll");
  };
  ...
  public dataStateChange(state: DataStateChangeEventArgs): void {
    ...
    let extraParams: any[] = [];
    this._coreService.execute(this.state, "hr/evaluation/GetAll", extraParams);
  }
  // Số thứ tự
  ...
  clickToolbar = (itemButton: any): void => {
    ...
    switch (buttonId) {
      ...
      case ToolbarItem.EXPORT_EXCEL:
        var data = this._coreService.ExecuteToExport('hr/evaluation/GetAll', '', this.state).subscribe((res: any) => {
          // Lấy ngày hiện tại
      ...
      case ToolbarItem.LOCK:
        if (selectDeletes && selectDeletes.length > 0) {
          ...
          this._coreService
            .Post("hr/evaluation/ChangeStatus", ids)
            .subscribe((res: any) => {
  ...
  confirmDelete = (status: any): void => {
    if (status === "cancel") {
      ...
      if (lstDeleteIds.length > 0) {
        this._coreService
          .Post("hr/evaluation/Delete", lstDeleteIds)
  ...
  saveData = () => {
    ...
    if (this.flagState$.value === Consts.add) {
      this._coreService.Post("hr/evaluation/add", this.model).subscribe(
        (res: any) => {
      ...
      this._coreService.Post("hr/evaluation/Update", this.model).subscribe(
        (res: any) => {
          if (res.statusCode == 400) {
            this.notification.checkErrorMessage(res.message);
          } else {
