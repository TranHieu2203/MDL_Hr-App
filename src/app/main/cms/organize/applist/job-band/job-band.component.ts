...
export class JobBandComponent implements OnInit {
  ...
  viewRecord = (event: any) => {
    ...
    this.router.navigate(["/hrms/organize/list/job-band/", paramAdd]);
  };
  ...
  getListData = () => {
    ...
    this._coreService.execute(this.state, "hr/job-band/job-bands");
  };
  ...
  clickToolbar = (itemButton: any): void => {
    ...
    switch (buttonId) {
      case ToolbarItem.ADD:
        this.dialogHeader = "Thêm mới cấp bậc";
        this.flagState$.next(Consts.add)
        this.model = new JobBand();
      ...
      case ToolbarItem.EXPORT_EXCEL:
        var data = this._coreService.ExecuteToExport('hr/job-band/job-bands','',this.state).subscribe((res: any) => {
          // Lấy ngày hiện tại
          ...
          const exportProperties: ExcelExportProperties = {
            dataSource: res.data,
            columns: [],
            fileName: "capbac" + "-" + formattedDate + ".xlsx"
          };
      ...
      case ToolbarItem.DELETE:
        if (selectDeletes && selectDeletes.length > 0) {
          ...
          for (let i = 0; i < this.modelDelete.length; i++) {
            if (this.modelDelete[i].statusName === "Áp dụng") {
              checkHieuLuc = true;
            }
          }
          if (checkHieuLuc) {
            this.notification.warning(
              "Hệ thống không cho phép xóa dữ liệu có trạng thái Áp dụng!"
            );
            return;
          }
      ...
      case ToolbarItem.LOCK:
        if (selectDeletes && selectDeletes.length > 0) {
          ...
          this._coreService
            .Post("hr/job-band/change-status", ids)
            .subscribe((res: any) => {
  ...
  confirmDelete = (status: any): void => {
    if (status === "cancel") {
      ...
      if (lstDeleteIds.length > 0) {
        this._coreService
          .Post("hr/job-band/delete", lstDeleteIds)
  ...
  saveData = () => {
    ...
    if (this.flagState$.value === Consts.add) {
      this._coreService.Post("hr/job-band/Update", this.model).subscribe(
        (res: any) => {
