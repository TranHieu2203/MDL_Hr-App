...
import { ToolbarItem, ToolbarInterface, Title } from "src/app/_models/index";
...
export class TitleComponent implements OnInit {
  ...
  constructor(
    ...
    this.editForm = this._formBuilder.group({
      ...
      nameEn: ["", []],
      titleGroupId: ["", [Validators.required]],
      code: [
        "",
        [
          Validators.required,
  ...
  viewRecord = (event: any) => {
    ...
    this.router.navigate(["/hrms/organize/list/title/", paramAdd]);
  };

  // GetListData
  getListData = (): void => {
    
    this._coreService.execute(this.state, "hr/title/GetAll");
  };
  ...
  public dataStateChange(state: DataStateChangeEventArgs): void {
    ...
    let extraParams: any[] = [];
    this._coreService.execute(this.state, "hr/title/GetAll", extraParams);
  }
  // Số thứ tự
  ...
  clickToolbar = (itemButton: any): void => {
    ...
    switch (buttonId) {
      ...
      case ToolbarItem.EXPORT_EXCEL:
        var data = this._coreService.ExecuteToExport('hr/title/GetAll','',this.state).subscribe((res: any) => {
          // Lấy ngày hiện tại
      ...
      case ToolbarItem.LOCK:
        if (selectDeletes && selectDeletes.length > 0) {
          ...
          this._coreService
            .Post("hr/title/ChangeStatus", ids)
            .subscribe((res: any) => {
  ...
  confirmDelete = (status: any): void => {
    if (status === "cancel") {
      ...
      if (lstDeleteIds.length > 0) {
        this._coreService
          .Post("hr/title/Delete", lstDeleteIds)
