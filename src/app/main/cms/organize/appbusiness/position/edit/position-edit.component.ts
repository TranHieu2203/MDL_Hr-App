...
export class PositionEditComponent implements OnInit {
  ...
  loadDataCombobx() {
    Promise.all([this.getListJob(),this.getListWorking(),this.getListTDCM(),this.getListLanguage(),this.getListComputer(),this.getListTitle()]).then((res: any) => {     
      this.lstJobId = res[0];
  ...
  convertModel(param: any) {
    ...
    model.effectivedate = model.effectivedate
      ? moment(model.effectivedate).format("YYYY-MM-DD")
      : null;
    return model;
  }
  // change date
