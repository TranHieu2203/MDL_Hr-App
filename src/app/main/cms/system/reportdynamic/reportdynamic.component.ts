...
@Component({
  selector: "cms-app-reportdynamic",
  templateUrl: "./reportdynamic.component.html",
  styleUrls: ["./reportdynamic.component.scss"],
...
export class ReportDynamicComponent implements OnInit {
  ...
  viewRecord = (event: any) => {
    ...
    this.router.navigate(["/hrms/system/reportdynamic/", paramAdd]);
  };

  configData = (id: any) => {


    const objParamAdd = { id: id, type: "edit" };
    const paramAdd = window.btoa(JSON.stringify(objParamAdd));
    this.router.navigate(["/hrms/system/reportdynamic/configdata", paramAdd]);
  }

  // Build Toolbar
  // buildToolbar = () => {
  //   const toolbarList = [ToolbarItem.ADD, ToolbarItem.EDIT, ToolbarItem.LOCK];
  //   this.toolbar = this.globals.buildToolbar("sys_groupuser", toolbarList!);
  // };

  // GetListData
  getListData = (): void => {
    
    this._coreService.execute(this.state, "package/reportdynamic/GetAll");
  };
  ...
  public dataStateChange(state: DataStateChangeEventArgs): void {
    ...
    let extraParams: any[] = [];
    this._coreService.execute(this.state, "package/reportdynamic/GetAll", extraParams);
  }
  // Số thứ tự
  ...
  clickToolbar = (itemButton: any): void => {
    ...
    switch (buttonId) {
      case ToolbarItem.ADD:
        this.router.navigate(["/hrms/system/reportdynamic/new"]);
        break;
      ...
      case ToolbarItem.LOCK:
        if (selectDeletes && selectDeletes.length > 0) {
          ...
          this._coreService
            .Post("tenant/group/changestatus", ids)
            .subscribe((res: any) => {
