    ...
    async.waterfall(
      [
        (cb: any) => {
          if (this.paramId) {
            this._coreService
              .Get("tenant/group/getbyid?id=" + this.paramId)
              .subscribe((res: any) => {
    ...
    switch (buttonId) {
      case ToolbarItem.BACK:
        ...
        if (this.flagePopup === true) {
          this.router.navigate(["hrms/system/groupuser"]);
        }
        break;
  ...
  saveData = () => {
    if (!this.editForm.valid) {
      ...
      if (this.flagState$.value === "new") {    
        this._coreService.Post("tenant/group/create", this.model).subscribe(
          (res: any) => {
            //check error
        ...
        this._coreService.Post("tenant/group/update", this.model).subscribe(
          (res: any) => {
            //check error
            if (res.statusCode == 400) {
              this.notification.checkErrorMessage(res.message);
