...
@Component({
  ...
  templateUrl: "./functionmenu.component.html",
  styleUrls: ["./functionmenu.component.scss"],
...
export class FunctionMenuComponent implements OnInit {
  ...
  hiddenItems: { id: number, isHiden: boolean }[] = [];
  ...
  viewRecord = (event: any) => {
    ...
    this.router.navigate(["/hrms/system/functionmenu/", paramAdd]);
  };

  // Build Toolbar
  buildToolbar = () => {
    const toolbarList = [ToolbarItem.LOCK];
  ...
  getListData = (): void => {
    ...
    this._coreService.execute(this.state, "tenant/getallfunction");
  };
  ...
  public dataStateChange(state: DataStateChangeEventArgs): void {
    ...
    let extraParams: any[] = [];
    this._coreService.execute(this.state, "tenant/getallfunction", extraParams);
  }
  // Số thứ tự
  ...
  clickToolbar = (itemButton: any): void => {
    ...
    switch (buttonId) {
      case ToolbarItem.ADD:
        ...
        this.router.navigate(["/hrms/system/function/new"]);
        break;
      ...
      case ToolbarItem.SAVE:
        ...
        if (this.hiddenItems.length == 0) {
          this.notification.warning("Chưa có thay đổi nào!");
          return;
        }
        else {
          this._coreService
            .Post("author/Function/UpdateIsHiden", this.hiddenItems)
            .subscribe((res: any) => {
              ...
              this.hiddenItems.splice(0, this.hiddenItems.length);
  ...
  toggleTempVisibility(row: any) {

    const existingItemIndex = this.hiddenItems.findIndex(item => item.id === row.id);

    if (existingItemIndex !== -1) {
      this.hiddenItems.splice(existingItemIndex, 1);
    } else {
      this.hiddenItems.push({ id: row.id, isHiden: row.isHiden });
    }
