...
import { ToolbarItem, ToolbarInterface, SalaryElement } from "src/app/_models/index";
...
export class SalaryElementComponent implements OnInit {
  ...
  viewRecord = (event: any) => {
    ...
    this.router.navigate(["/hrms/payroll/list/salaryelement/", paramAdd]);
  };
  ...
  getListData = (groupId: any): void => {
    ...
    this._coreService.execute(this.state, "payroll/element/GetAll", extraParams);
  };
  ...
  clickToolbar = (itemButton: any): void => {
    ...
    switch (buttonId) {
      ...
      case ToolbarItem.EXPORT_EXCEL:
        var data = this._coreService.ExecuteToExport('payroll/element/GetAll','',this.state).subscribe((res: any) => {
          // Lấy ngày hiện tại
          ...
          const exportProperties: ExcelExportProperties = {
            dataSource: res.data,
            columns: [],
            fileName: "phantuluong" + "-" + formattedDate + ".xlsx"
          };
  ...
  confirmDelete = (status: any): void => {
    if (status === "cancel") {
      ...
      if (lstDeleteIds.length > 0) {
        this._coreService
          .Post("payroll/element/delete", lstDeleteIds)
  ...
  public groupSalary() {
    this._coreService
      .Get("payroll/element/getlistgroup")
      .subscribe((res: any) => {
  ...
  saveData = () => {
    ...
    if (this.flagState$.value === Consts.add) {
      this._coreService.Post("payroll/element/add", this.model).subscribe(
        (res: any) => {
      ...
      this._coreService.Post("payroll/element/Update", this.model).subscribe(
        (res: any) => {
          if (res.statusCode == 400) {
            this.notification.checkErrorMessage(res.message);
          } else {
