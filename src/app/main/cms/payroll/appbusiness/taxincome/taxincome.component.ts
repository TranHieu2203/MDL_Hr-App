...
import { TaxIncome } from "src/app/_models/app/cms/payroll/appbusiness/taxincome";
setCulture("en");
setCurrencyCode("TRY");
@Component({
  selector: "cms-profile-taxincome",
  templateUrl: "./taxincome.component.html",
  styleUrls: ["./taxincome.component.scss"],
...
export class TaxIncomeComponent implements OnInit {
  ...
  getListColumn() {
    return new Promise((resolve) => {
      this._coreService
        .Get(
          "payroll/structure/getlist?SalaryTypeId=1508"
        )
        .subscribe((res: any) => {
  ...
  clickToolbar = (itemButton: any): void => {
    ...
    switch (buttonId) {
      case ToolbarItem.PAYROLL_CAL:
        ...
        this._coreService
          .Post("payroll/formula/TaxIncomeCal", this.model)
          .subscribe((res: any) => {
            if (res.statusCode == 400) {
              this.notification.checkErrorMessage(res.error);
            } else {
              this.notification.success("Tính thuế thành công");
              this.getListData();
            }
      ...
      case ToolbarItem.EXPORT_EXCEL:
        ...
        this._coreService
          .GetAll(state, "payroll/formula/ListTaxIncome", extraParams)
          .subscribe((res: any) => {
