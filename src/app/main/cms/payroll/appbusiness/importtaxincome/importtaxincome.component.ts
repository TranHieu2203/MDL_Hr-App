...
@Component({
  selector: "cms-importtaxincome",
  templateUrl: "./importtaxincome.component.html",
  styleUrls: ["./importtaxincome.component.scss"],
  providers: [FilterService, VirtualScrollService],
  encapsulation: ViewEncapsulation.None,
})
export class ImportTaxincomeComponent implements OnInit, OnDestroy {
  ...
  async changeSalaryTypeId(e: any) {
  ...
  search() {
    if (this.model.orgId && this.model.yearId) {
      this.getListData();
    }
  }
  // GetListData
    ...
    switch (buttonId) {
      ...
      case ToolbarItem.EXPORT_TEMPLATE:
        let paramExport = {
          ...
          SalTypeId: Consts.salarytype
        }
        this._coreService
          .templateImportV2Hrm("TemplateImportTaxation", paramExport)
          .subscribe((res: any) => {
      ...
      case ToolbarItem.EXPORT_EXCEL:
        ...
        var data = this._coreService.ExecuteToExport('hr/salimport/GetAll', extraParams, this.state).subscribe((res: any) => {
          ...
          const exportProperties: ExcelExportProperties = {
            dataSource: res.data,
            columns: [],
            fileName: "importquyettoanthue" + "-" + formattedDate + ".xlsx"
          };
    ...
    if (status === "cancel") {
      ...
      if (lstDeleteIds.length > 0) {
        let param = {
          ...
          periodId: this.model.periodId,
          year: this.model.yearId,
          salaryTypeId: Consts.salarytype
        }
        this._coreService
  ...
  getListColumn() {
    return new Promise((resolve) => {
      this._coreService
        .Get(
          "payroll/structure/GetListImport?SalaryTypeId=" + Consts.salarytype
        )
        .subscribe((res: any) => {
  ...
  confirmImport = (status: any): void => {
    if (status === "cancel") {
      ...
      var year = this.model.yearId?.toString();
      data.append("type_sal", JSON.stringify(1508));
      data.append("year", JSON.stringify(this.model.yearId));
