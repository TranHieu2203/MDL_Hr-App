...
import { CompetencyFrame } from "src/app/_models/app/cms/index";
...
@Component({
  selector: "app-competencyframe-edit",
  templateUrl: "./competencyframe-edit.component.html",
  styleUrls: ["./competencyframe-edit.component.scss"],
  providers: [FilterService, VirtualScrollService],
  encapsulation: ViewEncapsulation.None,
})
export class CompetencyFrameEditComponent implements OnInit {
  ...
  constructor(
    ...
    this.editForm = this._formBuilder.group({
      ...
      capacityGroupId: ["", [Validators.required, this.validateCapacityGroupId]],
      startDate :["",[]],
      endDate :["",[]],
      remark : ["",[]],
      note: [""],
  ...
  validateCapacityGroupId(control: AbstractControl) {
    const value = control.value;
    if (value === null) {

      return { invalidCapacityGroupId: true };
    }
    return null;
  }
  /**
    ...
    async.waterfall(
      [
        (cb: any) => {
          if (this.paramId) {
            this._coreService
              .Get("hr/competencyframe/get?id=" + this.paramId)
              .subscribe((res: any) => {
    ...
    switch (buttonId) {
      case ToolbarItem.BACK:
        ...
        if (this.flagePopup === true) {
          this.router.navigate(["hrms/capacity/list/competencyframe"]);
        }
        break;
  ...
  saveData = () => {
    ...
    if (this.flagState$.value === "new") {    
      this._coreService.Post("hr/competencyframe/add", param).subscribe(
        (res: any) => {
          //check error
      ...
      this._coreService.Post("hr/competencyframe/Update", param).subscribe(
        (res: any) => {
          //check error
          if (res.statusCode == 400) {
            this.notification.checkErrorMessage(res.message);
