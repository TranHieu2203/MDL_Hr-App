...
@Component({
  selector: "cms-app-compesetposition",
  templateUrl: "./compesetposition.component.html",
  styleUrls: ["./compesetposition.component.scss"],
  providers: [FilterService, VirtualScrollService],
  encapsulation: ViewEncapsulation.None,
})
export class CompeSetPositionComponent implements OnInit {
  ...
  viewRecord = (event: any) => {
    ...
    this.router.navigate(["/hrms/capacity/list/compesetposition/", paramAdd]);
  };

  // Build Toolbar
  buildToolbar = () => {
    const toolbarList = [ToolbarItem.ADD, ToolbarItem.EDIT, ToolbarItem.LOCK];
    this.toolbar = this.globals.buildToolbar("compesetposition", toolbarList);
  };
  // filter checkbox
  ...
  getListData = (): void => {
    ...
    this._coreService.execute(this.state, "hr/compesetposition/GetAll");
  };
  ...
  public dataStateChange(state: DataStateChangeEventArgs): void {
    ...
    let extraParams: any[] = [];
    this._coreService.execute(this.state, "hr/compesetposition/GetAll", extraParams);
  }
  // Số thứ tự
  ...
  clickToolbar = (itemButton: any): void => {
    ...
    switch (buttonId) {
      case ToolbarItem.ADD:
        this.router.navigate(["/hrms/capacity/list/compesetposition/new"]);
        break;
      case ToolbarItem.EDIT:
        ...
        if (selectRows && selectRows.length > 0) {
          ...
          this.router.navigate([
            "/hrms/capacity/list/compesetposition/",
            paramAdd,
          ]);
        } else {
      ...
      case ToolbarItem.EXPORT_EXCEL:
        var data = this._coreService.ExecuteToExport('hr/compesetposition/GetAll','',this.state).subscribe((res: any) => {
          // Lấy ngày hiện tại
          ...
          const exportProperties: ExcelExportProperties = {
            dataSource: res.data,
            columns: [],
            fileName: "chuannanglucchovitri" + "-" + formattedDate + ".xlsx"
          };
      ...
      case ToolbarItem.LOCK:
        if (selectDeletes && selectDeletes.length > 0) {
          ...
          this._coreService
            .Post("hr/compesetposition/ChangeStatus", ids)
            .subscribe((res: any) => {
  ...
  confirmDelete = (status: any): void => {
    if (status === "cancel") {
      ...
      if (lstDeleteIds.length > 0) {
        this._coreService
          .Post("hr/compesetposition/delete", lstDeleteIds)
