...
import { CompetencyPeriod } from "src/app/_models/app/cms/index";
...
@Component({
  selector: "app-competencyperiod-edit",
  templateUrl: "./competencyperiod-edit.component.html",
  styleUrls: ["./competencyperiod-edit.component.scss"],
  providers: [FilterService, VirtualScrollService],
  encapsulation: ViewEncapsulation.None,
})
export class CompetencyPeriodEditComponent implements OnInit {
    ...
    async.waterfall(
      [
        (cb: any) => {
          if (this.paramId) {
            this._coreService
              .Get("hr/competencyperiod/get?id=" + this.paramId)
              .subscribe((res: any) => {
    ...
    switch (buttonId) {
      case ToolbarItem.BACK:
        ...
        if (this.flagePopup === true) {
          this.router.navigate(["hrms/capacity/list/competencyperiod"]);
        }
        break;
  ...
  saveData = () => {
    ...
    if (this.flagState$.value === "new") {    
      this._coreService.Post("hr/competencyperiod/add", param).subscribe(
        (res: any) => {
          //check error
      ...
      this._coreService.Post("hr/competencyperiod/Update", param).subscribe(
        (res: any) => {
          //check error
          if (res.statusCode == 400) {
            this.notification.checkErrorMessage(res.message);
