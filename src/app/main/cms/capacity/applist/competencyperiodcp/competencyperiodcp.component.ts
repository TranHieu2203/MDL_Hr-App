...
import { ToolbarItem, ToolbarInterface, CompetencyPeriodCp } from "src/app/_models/index";
...
@Component({
  selector: "cms-app-competencyperiodcp",
  templateUrl: "./competencyperiodcp.component.html",
  styleUrls: ["./competencyperiodcp.component.scss"],
  providers: [FilterService, VirtualScrollService],
  encapsulation: ViewEncapsulation.None,
})
export class CompetencyPeriodCpComponent implements OnInit {
  ...
  // edit
  model: CompetencyPeriodCp = new CompetencyPeriodCp();
  modelTemp: CompetencyPeriodCp = new CompetencyPeriodCp();
  ...
  viewRecord = (event: any) => {
    ...
    this.router.navigate(["/hrms/capacity/list/competencyperiodcp/", paramAdd]);
  };

  // Build Toolbar
  buildToolbar = () => {
    const toolbarList = [ToolbarItem.ADD, ToolbarItem.EDIT, ToolbarItem.LOCK];
    this.toolbar = this.globals.buildToolbar("competencyperiodcp", toolbarList);
  };
  // filter checkbox
  ...
  getListData = (): void => {
    ...
    this._coreService.execute(this.state, "hr/competencyperiodcp/GetAll");
  };
  ...
  public dataStateChange(state: DataStateChangeEventArgs): void {
    ...
    let extraParams: any[] = [];
    this._coreService.execute(this.state, "hr/competencyperiodcp/GetAll", extraParams);
  }
  // Số thứ tự
  ...
  clickToolbar = (itemButton: any): void => {
    ...
    switch (buttonId) {
      case ToolbarItem.ADD:
        this.router.navigate(["/hrms/capacity/list/competencyperiodcp/new"]);
        break;
      case ToolbarItem.EDIT:
        ...
        if (selectRows && selectRows.length > 0) {
          ...
          this.router.navigate([
            "/hrms/capacity/list/competencyperiodcp/",
            paramAdd,
          ]);
        } else {
      ...
      case ToolbarItem.EXPORT_EXCEL:
        var data = this._coreService.ExecuteToExport('hr/competencyperiodcp/GetAll','',this.state).subscribe((res: any) => {
          // Lấy ngày hiện tại
          ...
          const exportProperties: ExcelExportProperties = {
            dataSource: res.data,
            columns: [],
            fileName: "kydanhgiacheckpoint" + "-" + formattedDate + ".xlsx"
          };
      ...
      case ToolbarItem.LOCK:
        if (selectDeletes && selectDeletes.length > 0) {
          ...
          this._coreService
            .Post("hr/competencyperiodcp/ChangeStatus", ids)
            .subscribe((res: any) => {
  ...
  confirmDelete = (status: any): void => {
    if (status === "cancel") {
      ...
      if (lstDeleteIds.length > 0) {
        this._coreService
          .Post("hr/competencyperiodcp/delete", lstDeleteIds)
