...
import { CompetencyPeriodCp } from "src/app/_models/app/cms/index";
...
@Component({
  selector: "app-competencyperiodcp-edit",
  templateUrl: "./competencyperiodcp-edit.component.html",
  styleUrls: ["./competencyperiodcp-edit.component.scss"],
  providers: [FilterService, VirtualScrollService],
  encapsulation: ViewEncapsulation.None,
})
export class CompetencyPeriodCpEditComponent implements OnInit {
    ...
    async.waterfall(
      [
        (cb: any) => {
          if (this.paramId) {
            this._coreService
              .Get("hr/competencyperiodcp/get?id=" + this.paramId)
              .subscribe((res: any) => {
        ...
        (cb: any) => {
          this._coreService.Get("hr/otherlist/GetTypePeriod").subscribe((res: any) => {
            this.lsttypePeriod = res.data;
            cb();
          });
        },
      ],
    ...
    switch (buttonId) {
      case ToolbarItem.BACK:
        ...
        if (this.flagePopup === true) {
          this.router.navigate(["hrms/capacity/list/competencyperiodcp"]);
        }
        break;
  ...
  saveData = () => {
    ...
    if (this.flagState$.value === "new") {    
      this._coreService.Post("hr/competencyperiodcp/add", param).subscribe(
        (res: any) => {
          //check error
      ...
      this._coreService.Post("hr/competencyperiodcp/Update", param).subscribe(
        (res: any) => {
          //check error
          if (res.statusCode == 400) {
            this.notification.checkErrorMessage(res.message);
